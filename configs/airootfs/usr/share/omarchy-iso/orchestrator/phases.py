...
def run(ctx: InstallContext, phases: list[tuple[str, PhaseFn]]) -> None:
    ...
    for index, (name, fn) in enumerate(phases):
        ...
        info(f"› {name}")
        # Wall clock stays for the dashboard and the log; the competitive
        # number is monotonic, so an NTP step or a timezone fix mid-install
        # cannot shorten or lengthen a phase.
        started = time.time()
    ...
    # The result is the sum of the phases, not the wall clock between the
    # first and the last: what the installer did, not what the clock said.
    state["total_elapsed_ns"] = sum(phase["elapsed_ns"] for phase in state["phases"])
