"""Unit tests for the install timing artifact (orchestrator/leaderboard.py) and
the schema-1 fields the phase loop records for it.
...
class PhaseLoopSchemaTest(LeaderboardFixture):
    def test_records_schema_run_id_ids_and_monotonic_ns(self):
        ...
        self.assertEqual(doc["total_elapsed_ns"], sum(p["elapsed_ns"] for p in doc["phases"]))
